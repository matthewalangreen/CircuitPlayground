...
void draw()
{
  ...
  // Find position on screen using accelerometer y and x
  // I'm using the acceleromoter Y values (in portValues[1]) to control
  // the x values on screen and the accelerometer X values (in portValues[0])
  // to control the y values on screen.  I'm flipping them becuase it makes
  // it easier to hold the circuit playground with the cable facing the monitor
  float x = map(portValues[1],-10,10,0,width);
  float y = map(portValues[0],-10,10,0,height);
  
  // create a new dot if left button is pressed
  if(portValues[3]==1) {
   makeDotHere(x,y); 
  }
  
  // Make dots move to their polar curve if right button pressed
  if(portValues[4]==1) {
   moving = !moving; 
  }
  
  // change to new polar curve if light sensor values drop
  // below 10
  if(portValues[5]<10) {
